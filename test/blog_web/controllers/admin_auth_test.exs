defmodule BlogWeb.AdminAuthTest do
  ...
  @remember_me_cookie "_blog_web_admin_remember_me"
  ...
  describe "log_in_admin/3" do
    test "stores the admin token in the session", %{conn: conn, admin: admin} do
      ...
      assert BlogDashboard.get_admin_by_session_token(token)
    end
  ...
  describe "logout_admin/1" do
    test "erases session and cookies", %{conn: conn, admin: admin} do
      admin_token = BlogDashboard.generate_admin_session_token(admin)
      ...
      assert redirected_to(conn) == "/"
      refute BlogDashboard.get_admin_by_session_token(admin_token)
    end
